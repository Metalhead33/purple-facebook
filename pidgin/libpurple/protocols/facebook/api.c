...
static void
fb_api_cb_seqid(PurpleHttpConnection *con, PurpleHttpResponse *res, gpointer data)
{
    const gchar *str;
    FbApi *api = data;
    FbApiPrivate *priv = api->priv;
...
static void
fb_api_cb_attach(PurpleHttpConnection *con, PurpleHttpResponse *res, gpointer data)
{
    const gchar *str;
    FbApi *api = data;
    FbApiMessage *msg;
...
fb_api_auth(FbApi *api, const gchar *user, const gchar *pass, const gchar *credentials_type)
{
    ...
    if (!(priv->machine_id) || strlen(priv->machine_id) == 0) {
        fb_http_params_set_str(prms, "generate_machine_id", "1");
    } else {
        fb_http_params_set_str(prms, "machine_id", priv->machine_id);
    }
  ...
  if (priv->uid &&
        ...
        fb_http_params_set_int(prms, "uid", priv->uid);
        ...
        fb_http_params_set_str(prms, "first_factor", priv->login_first_factor);

        // twofactor_code
        fb_http_params_set_str(prms, "twofactor_code", priv->twofactor_code);

        // password actually now same as twofactor_code
        fb_http_params_set_str(prms, "password", priv->twofactor_code);

        // userid , same as uid. needs to be here for 2FA to work.
        fb_http_params_set_int(prms, "userid", priv->uid);
...
static void
fb_api_cb_contact(PurpleHttpConnection *con, PurpleHttpResponse *res, gpointer data)
{
    const gchar *str;
    FbApi *api = data;
    FbApiUser user;
...
static void
fb_api_cb_contacts(PurpleHttpConnection *con, PurpleHttpResponse *res, gpointer data)
{
    const gchar *cursor;
...
static void
fb_api_cb_unread_msgs(PurpleHttpConnection *con, PurpleHttpResponse *res, gpointer data)
{
    const gchar *body;
    const gchar *str;
    FbApi *api = data;
...
static void
fb_api_cb_sticker(PurpleHttpConnection *con, PurpleHttpResponse *res, gpointer data)
{
    FbApi *api = data;
    FbApiMessage *msg;
    FbJsonValues *values;
...
static void
fb_api_cb_thread(PurpleHttpConnection *con, PurpleHttpResponse *res, gpointer data)
{
    FbApi *api = data;
    FbApiThread thrd;
    GError *err = NULL;
...
static void
fb_api_cb_thread_create(PurpleHttpConnection *con, PurpleHttpResponse *res, gpointer data)
{
    const gchar *str;
    FbApi *api = data;
    FbId tid;
